...
export function CreativePreview({ invoice, config }: { invoice: InvoiceData; config?: TemplateConfig }) {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '2em', fontWeight: 800, letterSpacing: '0.06em' }}>{labels.invoiceTitle}</div>
        ...
        {vis.meta && (
          <div style={{ display: 'flex', gap: '1em', marginBottom: '2em', flexWrap: 'wrap' }}>
            <div style={{ backgroundColor: purpleLight, padding: '0.6em 1.2em', borderRadius: '2em', fontSize: '0.8em' }}>
              <span style={{ color: purple, fontWeight: 600 }}>{labels.issueDateLabel + ':'} </span>
              {invoice.issueDate ? new Date(invoice.issueDate).toLocaleDateString() : '-'}
            </div>
            <div style={{ backgroundColor: purpleLight, padding: '0.6em 1.2em', borderRadius: '2em', fontSize: '0.8em' }}>
              <span style={{ color: purple, fontWeight: 600 }}>{labels.dueDateLabel + ':'} </span>
              {invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : '-'}
            </div>
            {invoice.paymentTerms && (
              <div style={{ backgroundColor: purpleLight, padding: '0.6em 1.2em', borderRadius: '2em', fontSize: '0.8em' }}>
                <span style={{ color: purple, fontWeight: 600 }}>{labels.paymentTermsLabel + ':'} </span>
                {invoice.paymentTerms}
              </div>
            )}
            {invoice.poNumber && (
              <div style={{ backgroundColor: purpleLight, padding: '0.6em 1.2em', borderRadius: '2em', fontSize: '0.8em' }}>
                <span style={{ color: purple, fontWeight: 600 }}>{labels.poNumberLabel + ':'} </span>
                {invoice.poNumber}
              </div>
            )}
          </div>
        ...
        {vis.addresses && (
          <div style={{ display: 'flex', gap: '1.5em', marginBottom: '2.5em' }}>
            <div style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: '1em', padding: '1.5em', boxShadow: '0 2px 8px rgba(124,58,237,0.08)', border: '1px solid #EDE9FE' }}>
              <div style={{ fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', color: purple, letterSpacing: '0.1em', marginBottom: '0.5em' }}>{labels.billToLabel}</div>
            ...
            {invoice.hasShipTo && (
              <div style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: '1em', padding: '1.5em', boxShadow: '0 2px 8px rgba(124,58,237,0.08)', border: '1px solid #EDE9FE' }}>
                <div style={{ fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', color: purple, letterSpacing: '0.1em', marginBottom: '0.5em' }}>{labels.shipToLabel}</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'linear-gradient(135deg, ' + purple + ', ' + purpleMid + ')', color: '#FFFFFF' }}>
                  <th style={{ padding: '0.8em 1.2em', textAlign: 'left', fontSize: '0.78em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{labels.descriptionLabel}</th>
                  <th style={{ padding: '0.8em 1.2em', textAlign: 'center', fontSize: '0.78em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{labels.quantityLabel}</th>
                  <th style={{ padding: '0.8em 1.2em', textAlign: 'right', fontSize: '0.78em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{labels.rateLabel}</th>
                  <th style={{ padding: '0.8em 1.2em', textAlign: 'right', fontSize: '0.78em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{labels.amountLabel}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ width: '28em', backgroundColor: '#FFFFFF', borderRadius: '1em', padding: '1.2em 1.5em', boxShadow: '0 2px 8px rgba(124,58,237,0.08)', border: '1px solid #EDE9FE' }}>
              ...
              {(taxAmount > 0 || itemTax > 0) && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em' }}>
                  <span style={{ color: '#6B7280' }}>{labels.taxLabel}</span>
                  <span>{formatCurrency(taxAmount + itemTax, invoice.currency)}</span>
                </div>
              )}
              {discountAmount > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em' }}>
                  <span style={{ color: '#6B7280' }}>{labels.discountLabel}</span>
                  <span>-{formatCurrency(discountAmount, invoice.currency)}</span>
                </div>
              )}
              {invoice.shipping > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em' }}>
                  <span style={{ color: '#6B7280' }}>{labels.shippingLabel}</span>
          ...
          <div style={{ marginTop: '2.5em', display: 'flex', gap: '1.5em' }}>
            {invoice.notes && (
              <div style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: '1em', padding: '1.2em 1.5em', boxShadow: '0 2px 8px rgba(124,58,237,0.06)', border: '1px solid #EDE9FE' }}>
                <div style={{ fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', color: purple, letterSpacing: '0.1em', marginBottom: '0.4em' }}>{labels.notesLabel}</div>
                <div style={{ fontSize: '0.82em', color: '#6B7280', whiteSpace: 'pre-line' }}>{invoice.notes}</div>
              </div>
            )}
            {invoice.terms && (
              <div style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: '1em', padding: '1.2em 1.5em', boxShadow: '0 2px 8px rgba(124,58,237,0.06)', border: '1px solid #EDE9FE' }}>
                <div style={{ fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', color: purple, letterSpacing: '0.1em', marginBottom: '0.4em' }}>{labels.termsLabel}</div>
