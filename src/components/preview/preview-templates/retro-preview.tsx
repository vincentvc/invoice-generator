...
export function RetroPreview({ invoice }: { invoice: InvoiceData }) {
    ...
    <div style={{ padding: '3em', fontFamily: 'Georgia, Times New Roman, serif', color: '#451A03', fontSize: '1em', height: '100%', backgroundColor: '#FFFBEB', lineHeight: 1.6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px dotted #D97706', paddingBottom: '1.5em', marginBottom: '1.5em' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2.2em', margin: 0, color: '#92400E', fontStyle: 'italic', letterSpacing: '0.05em' }}>INVOICE</h1>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: '0.8em', color: '#78350F' }}>
            ...
            {invoice.paymentTerms && <div style={{ marginTop: '0.3em' }}>Terms: {invoice.paymentTerms}</div>}
          </div>
        </div>
      </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2em' }}>
        <thead>
          <tr>
            <th style={{ backgroundColor: '#92400E', color: '#FFFBEB', padding: '0.8em 1em', textAlign: 'left', fontStyle: 'italic' }}>Description</th>
            <th style={{ backgroundColor: '#92400E', color: '#FFFBEB', padding: '0.8em 1em', textAlign: 'center', fontStyle: 'italic', width: '8%' }}>Qty</th>
            <th style={{ backgroundColor: '#92400E', color: '#FFFBEB', padding: '0.8em 1em', textAlign: 'right', fontStyle: 'italic', width: '15%' }}>Rate</th>
            <th style={{ backgroundColor: '#92400E', color: '#FFFBEB', padding: '0.8em 1em', textAlign: 'right', fontStyle: 'italic', width: '15%' }}>Amount</th>
          </tr>
        </thead>
        <tbody>
        ...
        <div style={{ width: '24em', border: '2px dotted #D97706', padding: '1em' }}>
          ...
          {(taxAmount > 0 || itemTax > 0) && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em', color: '#78350F' }}>
              <span>Tax</span><span>{formatCurrency(taxAmount + itemTax, invoice.currency)}</span>
            </div>
          )}
          {discountAmount > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em', color: '#78350F' }}>
              <span>Discount</span><span>-{formatCurrency(discountAmount, invoice.currency)}</span>
            </div>
          )}
          {invoice.shipping > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.9em', color: '#78350F' }}>
              <span>Shipping</span><span>{formatCurrency(invoice.shipping, invoice.currency)}</span>
            </div>
          )}
