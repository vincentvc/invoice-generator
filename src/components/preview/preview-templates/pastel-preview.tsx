...
export function PastelPreview({ invoice }: { invoice: InvoiceData }) {
    ...
    <div style={{ padding: '3em', fontFamily: 'Inter, Helvetica Neue, sans-serif', color: '#1F2937', fontSize: '1em', height: '100%', backgroundColor: '#FDF2F8', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5em' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2em', margin: 0, color: '#EC4899', fontWeight: 300, letterSpacing: '0.15em' }}>INVOICE</h1>
        </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2em', backgroundColor: '#FFFFFF', borderRadius: '0.8em', overflow: 'hidden' }}>
        <thead>
          <tr>
            <th style={{ padding: '0.8em 1em', textAlign: 'left', fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#EC4899', backgroundColor: '#FDF2F8' }}>Description</th>
            <th style={{ padding: '0.8em 1em', textAlign: 'center', fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#EC4899', backgroundColor: '#FDF2F8', width: '8%' }}>Qty</th>
            <th style={{ padding: '0.8em 1em', textAlign: 'right', fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#EC4899', backgroundColor: '#FDF2F8', width: '15%' }}>Rate</th>
            <th style={{ padding: '0.8em 1em', textAlign: 'right', fontSize: '0.7em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#EC4899', backgroundColor: '#FDF2F8', width: '15%' }}>Amount</th>
          </tr>
        </thead>
        <tbody>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '2em' }}>
        <div style={{ width: '24em', backgroundColor: '#FFFFFF', padding: '1.2em', borderRadius: '0.8em' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.85em' }}>
            <span style={{ color: '#9CA3AF' }}>Subtotal</span><span>{formatCurrency(subtotal, invoice.currency)}</span>
          </div>
          {(taxAmount > 0 || itemTax > 0) && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.85em' }}>
              <span style={{ color: '#9CA3AF' }}>Tax</span><span>{formatCurrency(taxAmount + itemTax, invoice.currency)}</span>
            </div>
          )}
          {discountAmount > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.85em' }}>
              <span style={{ color: '#9CA3AF' }}>Discount</span><span>-{formatCurrency(discountAmount, invoice.currency)}</span>
            </div>
          )}
          {invoice.shipping > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4em 0', fontSize: '0.85em' }}>
              <span style={{ color: '#9CA3AF' }}>Shipping</span><span>{formatCurrency(invoice.shipping, invoice.currency)}</span>
            </div>
          )}
